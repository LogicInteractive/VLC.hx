...
class VLCPlayer
{
	...
	public var onOpening	: (VLCPlayer)->Void;
	public var onPlaying	: (VLCPlayer)->Void;
	public var onStopped	: (VLCPlayer)->Void;
	public var onPaused		: (VLCPlayer)->Void;
	public var onEndReached	: (VLCPlayer)->Void;
	public var onError		: (VLCPlayer)->Void;

	var eventQueue			: Array<LibVLC_EventType> = [];

	public function new(?source:String)
	{
		//Hm. force array pre-allocation because of non-threadsafe access from LibVLC Callbacks
		eventQueue.resize(32);
	...
	function update()
	{
		...
		return;
		for (i in 0...eventQueue.length)
			if (eventQueue[i]!=0)
			{
				triggerEvent(eventQueue[i]);
				eventQueue[i] = 0;
			}
	}

	inline function triggerEvent(e:LibVLC_EventType)
	{
		switch (e)
		{
			case LibVLC_EventType.mediaPlayerOpening:			if (onOpening!=null) onOpening(this);
			case LibVLC_EventType.mediaPlayerPlaying:			if (onPlaying!=null) onPlaying(this);
			case LibVLC_EventType.mediaListPlayerStopped:		if (onStopped!=null) onStopped(this);
			case LibVLC_EventType.mediaPlayerPaused:			if (onPaused!=null) onPaused(this);
			case LibVLC_EventType.mediaPlayerEndReached:		if (onEndReached!=null) onEndReached(this);
			case LibVLC_EventType.mediaPlayerEncounteredError:	if (onError!=null) onError(this);
			default:
		}
	}
...
*/
	...
	static public function lock(data:VoidStar, p_pixels:VoidStarStar):VoidStar
	{
		...
		untyped __cpp__('*p_pixels = ctx->get_value().pixeldata');
			...
			// ctx->imagemutex.lock();
			// *p_pixels = ctx->get_value().pixeldata;
		// ');
		return null;
	}
...
/* 		untyped __cpp__('
		...
		switch ( event.type )
		{
			...
			{
				vlcp.isPlaying = true;
				@:privateAccess vlcp.eventQueue.shift();
				@:privateAccess vlcp.eventQueue.push(event.type);
			}
